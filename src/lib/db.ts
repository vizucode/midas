...
const db = createClient({ url: env.TURSO_HOST, authToken: env.TURSO_TOKEN });
...
export async function saveConversationState(input: {
  chatId: string;
  lastIntent?: string | null;
  lastParams?: string | null;
  lastTool?: string | null;
...
export async function saveIntent(input: IntentRecord): Promise<void> {
  await db.execute({
